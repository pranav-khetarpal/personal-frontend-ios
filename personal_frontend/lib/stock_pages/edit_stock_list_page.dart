...
class _EditStockListPageState extends State<EditStockListPage> {
  ...
  Future<void> showIntroMessage() async {
    ...
    bool isFirstTime = prefs.getBool('isFirstTimeEditingStockList') ?? true;

    if (isFirstTime) {
      await showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text("Stock List Editing"),
          content: const Text("You can change the stocks in the list by deleting stocks and searching for new ones to add. "
            "You can also change the order of the stocks shown by dragging and dropping stocks. And finally, you can change the stock list's name."),
          actions: [
            TextButton(
              onPressed: () {
      ...
      await showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text("Press the check mark at the top right to save your changes."),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
      ...
      await prefs.setBool('isFirstTimeEditingStockList', false);
    }
  }
